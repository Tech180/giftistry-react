...
Editable body`.length}
    ...
    expect(container.querySelector('[class*="section-divider__title"]')).toHaveTextContent(
      'Populate Prompt'
    );
    expect(container.querySelectorAll('[class*="section-divider__title"]')).toHaveLength(3);
    expect(container.querySelector('[class*="code-line--read-only"]')).not.toBeNull();
  });
