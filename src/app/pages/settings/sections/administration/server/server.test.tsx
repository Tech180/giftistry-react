...
import { writeLocalAiModelsCache } from 'features/system';
...
describe('Server local AI validation', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    localStorage.clear();
    vi.mocked(systemApi.getSettings).mockResolvedValue({ ...defaultSettings });
  ...
  test('loads and saves scrape timeout settings', async () => {
    ...
    const postCall = vi.mocked(systemApi.updateSettings).mock.calls.at(-1);
...
describe('Server OpenRouter models proxy', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    localStorage.clear();
    vi.mocked(systemApi.getSettings).mockResolvedValue({
...
describe('Server AI default prompts', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    localStorage.clear();
    vi.mocked(systemApi.getSettings).mockResolvedValue({ ...defaultSettings });
...
describe('Server AI disable persists config', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    localStorage.clear();
    vi.mocked(systemApi.getSettings).mockResolvedValue({
...
describe('Server metadata packs', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    localStorage.clear();
    vi.mocked(systemApi.getSettings).mockResolvedValue({ ...defaultSettings });
  ...
  async function submitSettings() {
    ...
    return vi.mocked(systemApi.updateSettings).mock.calls[0];
  }
  ...
  test('custom fragment is editable and built-in fragment stays readonly', async () => {
    ...
    const postCall = await submitSettings();
    expect(postCall![0]?.AiCustomPacks?.[0]?.PromptFragment).toBe('Updated book rules.');
  });
