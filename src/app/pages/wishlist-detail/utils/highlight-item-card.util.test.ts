...
import { highlightItemCard, peekHighlightItemCard } from './highlight-item-card.util';

describe('highlightItemCard', () => {
  afterEach(() => {
  ...
  it('scrolls to the card and toggles the highlight class', () => {
    ...
    highlightItemCard('gift-1', 'is-highlighted', 1500);
  ...
  it('does nothing when the card is missing', () => {
    expect(() => highlightItemCard('missing', 'is-highlighted', 1500)).not.toThrow();
  });
});

describe('peekHighlightItemCard', () => {
  afterEach(() => {
    document.body.innerHTML = '';
    vi.restoreAllMocks();
    vi.useRealTimers();
  ...
  it('waits for scroll settle then dwell before clearing highlight', async () => {
    ...
    const peekPromise = peekHighlightItemCard('gift-2', 'is-highlighted', {
      dwellMs: 2000,
  ...
  it('scrolls back to returnToItemId without a second highlight', async () => {
    ...
    const peekPromise = peekHighlightItemCard('gift-4', 'is-highlighted', {
      dwellMs: 500,
      scrollFallbackMs: 2000,
