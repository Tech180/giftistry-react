...
export const CommentEditor = forwardRef<CommentEditorHandle, EditorProps>(
  (
    ...
    const mentionCandidates =
      mentionQuery === null
        ? []
        : filterMentionCandidates(participants, mentionQuery, currentUserId);
    ...
    const handleMentionMouseEnter = (e: MouseEvent, userId: string, username: string) => {
    ...
    const applyMention = (index: number) => {
      ...
      if (!participant || !mentionRange || !editor) {
        return;
      }
    ...
    useEffect(() => {
      ...
      if (resolvedMode === 'hiddenFromOwner' && listOwnerId) {
        if (previousMode === 'hiddenFromOwner') {
          return;
        }
        ...
        if (previousMode === 'visibleToSelected' && demoteIds.length === 0) {
          return;
        }
      } else {
        return;
