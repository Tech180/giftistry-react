...
vi.mock('features/items/providers/session', () => ({
  useItemsSession: () => ({ canShowAi: canShowAiRef.current, user: null }),
}));
...
function renderStrip(onImported = vi.fn()) {
  return render(
    <MemoryRouter>
      <Strip mode="create-list" isExpanded onImported={onImported} />
    </MemoryRouter>
  );
}
...
describe('Strip', () => {
  ...
  test('keeps a hidden file input when collapsed for menu browse', () => {
    const { container } = render(
      <MemoryRouter>
        <Strip mode="create-list" isExpanded={false} onImported={() => {}} />
      </MemoryRouter>
    );
  ...
  test('browse() opens the menu file input filtered to an extension', async () => {
    const ref = React.createRef<import('./interfaces/handle.interface').Handle>();
    const { container } = render(
      <MemoryRouter>
        <Strip ref={ref} mode="create-list" isExpanded={false} onImported={vi.fn()} />
      </MemoryRouter>
    );
