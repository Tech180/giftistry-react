import { POPULATE_HUB_HEADER_PATTERN } from '../constants/populate-hub-header-pattern.constant';
import { POPULATE_HUB_HEADERS } from '../constants/populate-hub-headers.constant';
import type { PopulateHubPromptSections } from '../interfaces/populate-hub-prompt-sections.interface';
...
export function parsePopulateHubPrompt(combined: string): PopulateHubPromptSections | null {
  ...
  const description = combined.slice(descIdx + descriptionMarker.length, catIdx).trim();
