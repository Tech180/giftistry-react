...
import styles from './ai-status-badge.module.css';
  ...
  const content = (
    <>
      ...
      <div className={styles['border-wrapper']}>
        <div className={styles['border-gradient']} aria-hidden="true" />

        <div className={styles.inner}>
          <div className={styles['icon-slot']} aria-hidden="true">
            <svg
              className={`${styles.icon} ${styles['icon-enabled']}`}
              viewBox="0 0 24 24"
              fill="none"
            ...
            <svg
              className={`${styles.icon} ${styles['icon-disabled']}`}
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
          ...
          {!isCompact && (
            <span className={`${styles.label} ${enabled ? styles['label-enabled'] : styles['label-disabled']}`}>
              {displayLabel}
            </span>
          )}
        </div>
    ...
    <>
      ...
      {isInteractive ? (
        <button
          ...
          onClick={onToggle}
          disabled={disabled}
          aria-pressed={label ? undefined : enabled}
